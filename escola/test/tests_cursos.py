...
class CursosTestCase(APITestCase):
    ...
    #def test_fail(self):
    #    self.fail("Press F to pay respect.") 
    
    def test_requisicao_get_para_listar_cursos(self):
        """Teste de validação da requisição GET"""
        response = self.client.get(self.list_url)
        self.assertEquals(response.status_code, status.HTTP_200_OK)
        
    def test_requisicao_post_para_criar_cursos(self):
        """Teste de validação da requisição POST"""
        data = {
            'codigo_curso':'CTT3',
    ...
    def test_requisicao_delete_para_deletar_curso(self):
        """Teste para verificar requisição DELETE não permitida para deletar curso"""
        response = self.client.delete('/cursos/1/')
        self.assertEquals(response.status_code, status.HTTP_405_METHOD_NOT_ALLOWED)
        
    def test_requisicao_put_para_atualizar_curso(self):
        """Teste para verificar requisição PUT para atualizar curso"""
        data = {
            'codigo_curso': 'CTT1',
